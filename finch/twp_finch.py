...
def get_pred_dist(pred_loss_seq, method=np.sum):
    """

    :param pred_loss_seqs:
    :param method: np.sum, np.mean, np.max
    :return:
    """
    seq_len = len(pred_loss_seq)
    pred_dist = np.zeros(shape=[seq_len, seq_len])

    pred_loss_norm = (pred_loss_seq - np.min(pred_loss_seq)) / (np.max(pred_loss_seq) - np.min(pred_loss_seq))
    # pred_loss_norm = pred_loss_seq
    for i in range(seq_len):
        for j in range(i+1, seq_len, 1):
            s_idx, e_idx = min(i, j), max(i, j)

            pred_dist[j, i] = pred_dist[i, j] = method(pred_loss_norm[s_idx: e_idx])

    return pred_dist


def clust_rank(mat, initial_rank=None, distance='cosine', pred_dist_method=np.sum, finch_type="twp"):
    s = mat.shape[0]
    ...
    if initial_rank is not None:
        ...
        if finch_type == "twp":
            ...
            pred_dist = get_pred_dist(pred, pred_dist_method)

            orig_dist = metrics.pairwise.pairwise_distances(mat, mat, metric=distance)
            # orig_dist = orig_dist * loc_dist + pred_dist
            orig_dist = orig_dist * loc_dist + 0.1 * pred_dist
...
def req_numclust(c, data, req_clust, distance, pred_dist_method, finch_type="twp"):
    iter_ = len(np.unique(c)) - req_clust
    c_, mat = get_merge([], c, data)
    for i in range(iter_):
        adj, orig_dist = clust_rank(mat, initial_rank=None, distance=distance, pred_dist_method=pred_dist_method, finch_type=finch_type)
...
def FINCH(data, initial_rank=None, req_clust=None, distance='cosine', pred_dist_method=np.sum, finch_type="twp", ensure_early_exit=False,
          verbose=True):
    ...
    :param use_twp_finch: Run TWP_FINCH on video data.
    ...
    min_sim = None
    # TODO: enhance to better use of predictive loss
    adj, orig_dist = clust_rank(data, initial_rank, distance=distance, pred_dist_method=pred_dist_method, finch_type=finch_type)
    initial_rank = None
    ...
    while exit_clust > 1:
        adj, orig_dist = clust_rank(mat, initial_rank, distance=distance, pred_dist_method=pred_dist_method, finch_type=finch_type)
    ...
    if req_clust is not None:
        if req_clust not in num_clust:
            ...
            req_c = req_numclust(c[:, ind[-1]], data, req_clust, distance, pred_dist_method, finch_type=finch_type)
        else:
...
def main():
    ...
    start = time.time()
    c, num_clust, req_c = FINCH(data, initial_rank=None, req_clust=None, distance='cosine', ensure_early_exit=True,
                                verbose=True)
