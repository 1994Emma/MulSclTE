...
def clust_rank(mat, initial_rank=None, distance='cosine', pred_dist_method=np.sum, finch_type="tw"):
    s = mat.shape[0]
...
def req_numclust(c, data, req_clust, distance, pred_dist_method, finch_type):
...
def FINCH_AS_REFINER(data, initial_rank=None, req_clust=None, distance='cosine', pred_dist_method=np.sum, finch_type="tw", ensure_early_exit=False,
          verbose=True, init_group=None, init_num_clust=None):
    """ FINCH clustering algorithm, to refine the results of extreme points
    ...
    # Emma: changed to support using the given clusters to init
    group, num_clust = init_group, init_num_clust
...
def main():
    ...
    start = time.time()
    c, num_clust, req_c = FINCH_AS_REFINER(data, initial_rank=None, req_clust=None, distance='cosine', ensure_early_exit=True,
                                verbose=True)
